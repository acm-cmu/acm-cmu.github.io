...
function Popup({ event, onClose }) {
  ...
  // Accepts event.imagePopup as string or array
  const images = event.imagePopups;
    ...
    <div className="popup-overlay" onClick={onClose}>
      <div className="popup-content" onClick={(e) => e.stopPropagation()}>
        <div className="popup-slideshow">
          <button className="popup-arrow left" onClick={prevImage} disabled={images.length <= 1}>&lt;</button>
          <ResponsiveImage src={images[current]} alt={event.name} className="popup-image" />
          <button className="popup-arrow right" onClick={nextImage} disabled={images.length <= 1}>&gt;</button>
          {images.length > 1 && (
          <div className="popup-indicators">
            {images.map((_, idx) => (
              <span
                key={idx}
                className={"popup-indicator" + (idx === current ? " active" : "")}
